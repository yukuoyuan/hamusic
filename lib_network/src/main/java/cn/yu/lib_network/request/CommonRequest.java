...
public final class CommonRequest implements CommonRequestInterface {
    ...
    public Request createRequest(RequestParams requestParams) {
        ...
        switch (requestMethod) {
            ...
            case POSTJSON:
                return createPostJsonRequest(requestParams);
            case POSTFORM:
            default:
    ...
    public Request createMultipartRequest(RequestParams requestParams, DisposeDataHandle disposeDataHandle) {
        ...
        if (disposeDataHandle.mCommonResponseListener instanceof UploadFileListener) {
            /*
             * 如果是带上传进度的,那么就监听上传进度
             */
            multipartRequestBody = new MultipartRequestBody(multipartBodyBuilder.build(), disposeDataHandle);
        } else {
            /*
             * 不带进度的
             */
            multipartRequestBody = multipartBodyBuilder.build();
        }
        /*
         * 创建一个header,把头数据塞进去
         */
